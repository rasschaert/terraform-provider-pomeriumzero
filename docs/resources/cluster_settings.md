...
- `cluster_id` (String) The ID of the cluster these settings apply to. The Pomerium Zero settings endpoints are nested under the cluster (`/clusters/{cluster_id}/settings`), so this must be set to the target cluster's ID — typically `pomeriumzero_cluster.<name>.id`.
...
- `codec_type` (String) The codec type to use. Valid values are '', 'auto', 'http1', 'http2', 'http3'.
...
- `id` (String) The unique identifier of the cluster settings. This is set to the cluster ID.
