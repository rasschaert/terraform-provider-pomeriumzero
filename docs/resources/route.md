...
- `set_request_headers` (Map of String) Map of headers to set on requests before forwarding to the upstream service. Values support Pomerium's dynamic substitution syntax (e.g. `${pomerium.request.headers["X-Id-Token"]}`). Use `$$` to emit a literal `$`.
- `show_error_details` (Boolean) If set to `true`, shows detailed error messages when errors occur.
- `tls_downstream_server_name` (String) TLS Downstream Server Name overrides the hostname specified in the from field.
...
- `enforced_policy_ids` (List of String) List of policy IDs that are enforced on this route at the namespace or organization level (read-only).
