...
- `domain` (String) The domain associated with this cluster, used to generate the FQDN. The Pomerium Zero API assigns this automatically (adjective-animal-NNNN format) and rejects user-chosen values, so this field is read-only.
...
terraform import pomeriumzero_cluster.default my-cluster
```
